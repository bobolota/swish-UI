...
export function PlayByPlay({ events = [], onUndo, homeTeamId, currentConfig, readOnly = false }) {
  ...
  const translateEvent = (eventType) => {
    ...
    if (eventType === 'timeout') return 'Temps Mort';

    if (!currentConfig || !currentConfig.actions) return eventType;

    for (const action of currentConfig.actions) {
      if (action.type === eventType) return action.label;
      
      if (action.outcomes) {
        const matchingOutcome = action.outcomes.find(o => `${action.type}${o.suffix}` === eventType);
        if (matchingOutcome) return `${action.label} - ${matchingOutcome.label}`;
      }
    }
    return eventType;
  };

  // 1. On isole la génération des lignes d'actions
  ...
  // 👇 2. LA MAGIE EST ICI : En mode readOnly, on ne renvoie QUE la liste brute.
  // Le composant MatchSummary gère déjà le scroll et la hauteur !
  if (readOnly) {
    return (
  ...
  // 3. Mode Standard (Table de Marque) : Garde son propre cadre et son propre défilement
  return (
    <div className="flex flex-col h-full overflow-hidden bg-white rounded-2xl border border-slate-200 shadow-sm">
